...
fn init() -> bool {
    ...
    let private_key_bytes: Vec<u8> = private_key
        .split(',')
    ...
    let sender_pubkey = signer.pubkey(); // Get the sender's public key
    let lamports_to_send = 10_000_000;
    // Create the transfer instruction
    let instruction = solana_sdk::system_instruction::transfer(
        &sender_pubkey,          // Source public key (signer)
        &pubkey,         // Destination public key (your public key)
...
// Redeem function: send coins after the game ends.
// fn redeem(send_coins: u32) -> bool {
//     let pubkey = solana::signer(); // Get the player's public key.
...
//     let signer = Keypair::from_bytes(&private_key_bytes).expect("Invalid private key");
//     let sender_pubkey = signer.pubkey(); // Get the sender's public key
//     let lamports_to_send = send_coins as u64 * 10_000; // Convert game coins to lamports.
...
//         Some(&pubkey),        // Fee payer
//         &[&signer],                  // Signers
//         blockhash, // Fetch the latest blockhash
//     );

//     // Send the transaction
//     let did_send = client.send_and_confirm_transaction(&tx);

//     return true; // Return transaction status.
// }
